...
void BleFramePresenter::on_run_calc()
{
    ...
    auto solver = std::make_shared<ble_src::calc::models::BleCalc>();
    ...
    std::string mess = ble_src::common::services::string_format("calculation completed in %.2f sec.", diff.count());
...
std::tuple<std::string, ble_src::logging::SeverityLevelEnum> BleFramePresenter::parse_log_mess(std::string mess)
{
    auto get_time = [](const std::string mess) {
        std::vector<std::string> a = ble_src::common::services::split(mess, " ");
        std::string v = a[0].substr(1);
        std::string v2 = a[1].substr(0, a[1].size() - 1);
        return ble_src::common::services::string_format("%s %s", v.c_str(), v2.c_str());
    };
    ...
    std::string pp = ble_src::common::services::string_format("%s - %s", time.c_str(), mm.c_str());
